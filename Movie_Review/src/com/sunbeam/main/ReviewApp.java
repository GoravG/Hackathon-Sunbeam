							...
							while ((choice1 = mainMenu(sc)) != null) {
								...
								case WRITE_REVIEW: {
									...
									if (flagMovieFound != false) {
										...
										if(review.contains(" ")) {
										rDao.createReview(currentUser, availableMovies.get(index), review, rating);}
										else
											System.out.println("Review Cannot be One Word Long.");
									} else {
								...
								case EDIT_REVIEW: {
									...
									for (int i = 0; i < list.size(); i++) {
										System.out.printf("%s\t\t%s\t\t%s", list.get(i).getReviewId(),
									...
									if (reviewFound != false) {
										...
										rDao.editReview(currentUser.getUserId(), review, rating);
									} else {
								...
								case SHARE_REVIEW: {
									...
									if (reviewFlag != false) {
										...
										System.out.print("Enter the user ID to share the review with :: ");
										...
										if (userFlag != false) {
											int count = rDao.shareReview(review_id, user_id);
											if (count == 0)
